...
function display(questionsIndex) {
    ...
    for (var i = 0; i < userQuestions.length; i++) {
        let userQuestions = userQuestions[questionsIndex].question;
        var userAnswers = userQuestions[questionsIndex].multipleChoices;
...
function compare(event) {
    ...
    if (element.matches("li")) {
        ...
        if (element.textContent == userQuestions[questionsIndex].answer) {
            score++;
            createDiv.textContent = "Correct!   " + userQuestions[questionsIndex].answer;
        } else {
            startTime = startTime - timeSubtraction;
            createDiv.textContent = "Wrong! The correct answer is:  " + userQuestions[questionsIndex].answer;
        }
    }
    // Question Index determines which question user is on
    questionsIndex++;
    if (questionsIndex >= userQuestions.length) {
        finished();
    } else {
        display(questionsIndex);
    }
...
function finished() {
    ...
    if (startTime >= 0) {
        var startemaining = startTime;
        const createP2 = document.createElement("p");
        clearInterval(pauseInterval);
        createP.textContent = "Your final score is: " + startemaining * 2;
